...
def train(args):
    ...
    # sup data
    sup_dir = os.path.join(args.data_dir, 'sup_data')
    neighbor_dir = os.path.join(sup_dir, 'neighbor.json')
    split_dir = os.path.join(sup_dir, 'split.json')
    with open(neighbor_dir) as f:
        neighbors = json.load(f)

    dataset = ActionData(root=args.data_dir, data_type='train',  neighbors=None, split_dir=split_dir,
                         neighbor_pattern=args.neighbor_pattern, num_frames=num_frames, stride=stride)
    testset = ActionData(root=args.data_dir, data_type='test', neighbors=None, split_dir=split_dir,
                         neighbor_pattern=args.neighbor_pattern, num_frames=num_frames, stride=stride)

    dataloader = DataLoader(dataset, batch_size=args.b, shuffle=True, num_workers=args.j)
    ...
    for epoch in range(100):
        ...
        trainer.test(epoch, testloader, modify=args.modify)
