...
class CognitiveProcessor(nn.Module):
    def __init__(self, n_elements=25, input_dim=64, convert_type='indirect', mlp_in_edge=True, n_channels=8,
                 num_features=50, k=6, multi_var=False, differ_muk=False, muk_func=False, dis_type='None'):
        super().__init__()
        ...
        if dis_type == 'Gmm':
            self.dll = GmmDistribution(frame_size=100, k=k, multi_var=multi_var,
                                       differ_muk=differ_muk, muk_func=muk_func)
        elif dis_type == 'MultiGaussian':
            self.dll = MultiGaussian(frame_size=100, n_elements=n_elements)
